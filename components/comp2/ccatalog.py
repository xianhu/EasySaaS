...
def layout(catalog_list, curr_path=None, class_name=None):
    ...
    for title_first, id_first, list_second in catalog_list:
        if isinstance(list_second, str):
            ...
            cgitem_list.append(html.A(title_first, id=id_first, href=list_second, className=_class))
            continue

        # define components
        cgitem_list.append(html.Div(title_first, className=class_title))
        for title, _id, path in list_second:
            _class = class_curr if path == curr_path else class_none
            cgitem_list.append(html.A(title, id=_id, href=path, className=_class))

    # return result
    return html.Div(cgitem_list, className=f"d-flex flex-column {class_name}")
